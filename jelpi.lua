...
function _init()
  --TODO why is player made at y+1
  actor = {}
  sparkle = {}
  t = 0
  death_t = 0
  ...
  for y=0,63 do for x=0,127 do
    if (mget(x,y) == 48) then --> sprite 48 is player
...
function move_actor(pl)
  ...
  if (not solid(x1,pl.y-0.5)) then
    ...
    -- if charging, break block	
	  if pl.charge ~= nil and (pl.charge > 0 or pl.super > 0) then
  ...
  if (pl.dy < 0) then
    -- going up
    if (solid(pl.x-0.2, pl.y+pl.dy-1) or solid(pl.x+0.2, pl.y+pl.dy-1)) then
      pl.dy=0
      -- search up for collision point
      while ( not (solid(pl.x-0.2, pl.y-1) or solid(pl.x+0.2, pl.y-1))) do
        pl.y = pl.y - 0.01
      end
    else
      pl.y = pl.y + pl.dy
    ...
    -- going down
    if (solid(pl.x-0.2, pl.y+pl.dy) or solid(pl.x+0.2, pl.y+pl.dy)) then
      -- bounce
      if (pl.bounce > 0 and pl.dy > 0.2) then
      ...
      --snap down
      while (not (solid(pl.x-0.2,pl.y) or solid(pl.x+0.2,pl.y))) do
        pl.y = pl.y + 0.05
      end
  ...
  -- x friction
  if (pl.standing) then
 	  pl.dx = pl.dx * 0.8 --surface friction
	else
 	  pl.dx = pl.dx * 0.9 --air resistance
	end
...
function move_player(pl)
  ...
  -- charge
  if (btn(5,b) and pl.charge == 0 and pl.delay == 0) then
    pl.charge = 15
  ...
  -- charging
  if (pl.charge > 0 or pl.super  > 0) then
    pl.frame = 53
...
function trigger_death()
  death_t = 1 --> trigger death sequence
  for i=1,32 do
    s=make_sparkle(pl.x, pl.y-0.6, 96, 0)
    s.dx = cos(i/32)/2
    s.dy = sin(i/32)/2
    s.max_t = 30 
    s.ddy = 0.01
    s.frame=96+rnd(3) --> sparkle sprites are static
    s.col = 7
  end
  del(actor,pl)
  sfx(16)
...
function clear_cel(x, y)
  val0 = mget(x-1,y) --left neighbor
  val1 = mget(x+1,y) --right neighbor
  if (val0 == 0 or val1 == 0) then
    mset(x,y,0) --one or the other is empty
  elseif (not fget(val1,1)) then
    mset(x,y,val1)
    --right neighbor has not flag 1 set (orange)
    --right neighbor is non-ground
  else
    mset(x,y,val0)
  end
end
...
function collide_event(a1, a2)
  ...
  if(a2.kind==3) then -- monster
    if(a1.charge > 0 or a1.super  > 0 or (a1.y-a1.dy) < a2.y-0.7) then
      -- slow down player
      a1.dx = a1.dx * 0.7
      a1.dy = a1.dy * -0.7-- - 0.2
      -- explode
      for i=1,16 do
        s=make_sparkle(a2.x, a2.y-0.5, 96+rnd(3), 7)
...
function move_spawns(x0, y0)
  ...
  for y=0,32 do
    for x=x0-10,x0+10 do
      val = mget(x,y) --sprite number
      spawn_sprite(val, x, y)
    end
  end
end


function spawn_sprite(val, x, y)
  m = nil --need to declare outside if statements

  -- pickup
  if (fget(val, 5)) then --flag 5 is pickup  
  ...
  -- monster
  if (fget(val, 3)) then --flag 3 for monster
...
function outgame_logic()
  if (death_t > 0) then
    ...
    if (death_t > 30 and btn(4) or btn(5)) then 
      ...
      --> where the colors should fade down to
...
function _draw()
  -- sky
  camera (0, 0)
  cls(12)--rectfill (0,0,127,127,12) 
...
function draw_actor(pl)
  ...
  spr(pl.frame, pl.x*8-4, pl.y*8-8, 1, 1, pl.d < 0) 
  pal()
end